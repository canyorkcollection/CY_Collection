...
const photoToArtist: Record<string, string> = {
  'carlos_vega.webp': 'Carlos Vega', 'bob_knox.jpg': 'Bob Knox',
  'Leslie-Wayne.jpeg': 'Leslie Wayne', 'Keith_Mayerson-BW.jpg': 'Keith Mayerson',
  'Hank_Willis_Thomas.jpg': 'Hank Willis Thomas', 'Jason-fox.jpg': 'Jason Fox',
  'TomBurckhardt.jpg': 'Tom Burckhardt', 'Laura-Sharp-Wilson.jpg': 'Laura Sharp-Wilson',
  'Brad-Kahlhamer.jpg': 'Brad Kahlhamer', 'thordis-adalsteinsdottir.jpeg': 'Thordis Adalsteinsdottir',
  'W-S.jpg': 'W-S', 'Sean-Mellyn.png': 'Sean Mellyn', 'Alexi-Worth-.jpeg': 'Alexi Worth',
  'Tim-Lokiec-.jpg': 'Tim Lokiec', 'Jason-Middlebrook.png': 'Jason Middlebrook',
  'Chris-Hammerlein.jpg': 'Chris Hammerlein', 'Don-Porcaro-610x787-1.jpg': 'Don Porcaro',
  'Joseph-Stashkevetch.jpg': 'Joseph Stashkevetch', 'Kurt-Lightner.png': 'Kurt Lightner',
  'PHOT-robert-and-shana-parkeharrison-1.jpg': 'Robert & Shana ParkeHarrison',
  'Julie-Heffernan.jpg': 'Julie Heffernan', 'James-Hyde.png': 'James Hyde',
  'Nuno-de-Campos.png': 'Nuno de Campos', 'Justin-Lieberman-scaled.jpg': 'Justin Lieberman',
  'Lesley-Dill.jpg': 'Lesley Dill', 'claudette-schreuders.jpg': 'Claudette Schreuders',
  'Kerry-James-Marshall.jpg': 'Kerry James Marshall', 'Richard-Hambleton.jpg': 'Richard Hambleton',
}
...
async function downloadAndUpload(url: string, storagePath: string): Promise<boolean> {
  try {
    ...
    const { error } = await supabaseAdmin.storage.from('artwork-images').upload(storagePath, buffer, { contentType, upsert: true })
    return !error
  } catch { return false }
}
...
export async function GET(request: NextRequest) {
  ...
  const { data: artworks } = await supabaseAdmin.from('artworks').select('id, title, sort_order').order('sort_order')
  ...
  for (const { folder, files, month } of allFolders) {
    ...
    if (!artworkId) { results.errors.push(`No artwork for folder ${folder}`); continue }
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      if (ok) {
        ...
        const { error } = await supabaseAdmin.from('artwork_images').insert({ artwork_id: artworkId, url: imageUrl, type: imgType, sort_order: i })
